  ...
  const calculateResult = async () => {
    try {
      ...
      questions.forEach((question, index) => {
        if (
          question.correctOption === question.options[selectedOptions[index]]
        ) {
  ...
  return (
    examData && (
      <div className="mt-2">
        ...
        <h1 className="text-center font-sans text-3xl">{examData.name}</h1>
        <div className="divider"></div>
            ...
            <div className="flex flex-col gap-2 p-2">
              {Object.keys(questions[selectedQuestionIndex].options).map(
                (option, index) => {
                  return (
                    <div
                      className={`flex gap-2 flex-col rounded-lg m-2 hover:shadow-mg ${
